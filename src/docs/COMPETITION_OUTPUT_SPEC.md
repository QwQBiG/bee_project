# ⚠️ 旧版接口说明（已被 2026-09《作品提交要求》取代）

> 本文记录的逐图 `--image_path`、stdout JSON 接口**不得用于最终提交**。
> 最新规则要求四个独立程序通过 `--input "<图片文件夹>"` 一次处理完整序列，
> 并将汇总 JSON 写入 `C:/TestResults/`。当前公共源码入口为
> `python -m inference.batch_cli`，最终打包时再生成四个规定名称的 EXE。
> 本文以下内容仅保留为历史实现与评分背景，不再作为交付依据。
