...
以下流程面向 64 位 Windows 10/11 和 NVIDIA RTX 显卡。首次部署需要联网下载依赖；两个正式 ONNX 模型已随仓库提供。
...
拉取仓库后应已有以下模型；测试视频可放入所示目录，也可以使用任意绝对路径：

```text
weights/bee_inside_ft_v1.onnx
weights/hive_entrance_bee_yolov8n.onnx
src/data/external/oist/oist_M13_ir_test_10s.mp4
src/data/external/vnbee/vnbee_outside_test_2s.mp4
```
...
python src/main.py --mode inside --video ".\src\data\external\oist\oist_M13_ir_test_10s.mp4" --output ".\output\demo_inside" --config ".\configs\demo_inside_test.yaml" --device cuda:0

python src/main.py --mode outside --video ".\src\data\external\vnbee\vnbee_outside_test_2s.mp4" --output ".\output\demo_outside" --config ".\configs\demo_outside_test.yaml" --device cuda:0
```
...
GitHub 仓库直接包含以下正式 ONNX 推理权重：

| 文件 | 大小（字节） | SHA-256 |
|---|---:|---|
| `bee_inside_ft_v1.onnx` | 44,724,083 | 以仓库当前文件校验值为准 |
| `hive_entrance_bee_yolov8n.onnx` | 12,745,151 | 以仓库当前文件校验值为准 |

```powershell
Get-FileHash -Algorithm SHA256 -LiteralPath ".\weights\bee_inside_ft_v1.onnx", ".\weights\hive_entrance_bee_yolov8n.onnx"
```

测试视频和数据集应遵守原数据来源的授权条件，不应直接提交到公开仓库。
