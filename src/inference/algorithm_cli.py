...
def _default_search_paths(prog_path: Path) -> List[Path]:
    ...
    candidates = [
        # PyInstaller onedir stores bundled data below ``_internal`` and sets
        # sys._MEIPASS to that directory at runtime.
        *(([Path(frozen_root) / "configs" / DEFAULT_CONFIG_BASENAME])
          if frozen_root else []),
        prog_path.parent / "_internal" / "configs" / DEFAULT_CONFIG_BASENAME,
        ...
        Path(__file__).resolve().parents[2] / "configs" / DEFAULT_CONFIG_BASENAME,
    ]
...
def load_runtime_config(explicit: Optional[str] = None) -> Dict[str, Any]:
    ...
    The config schema is::
        ...
        {
          "detector": {
            "outside": {"model": "../weights/hive_entrance.onnx",
                        "imgsz": 1280, "conf": 0.25, "iou": 0.45,
                        "scene_hint": "outside"},
            "inside":  {"model": "../weights/honey_bee_detector.onnx",
