...
class AlgorithmCliStructureTest(unittest.TestCase):
    ...
    def test_onedir_config_is_found_under_internal(self):
        from inference.algorithm_cli import (
            DEFAULT_CONFIG_BASENAME, _default_search_paths,
        )
        with tempfile.TemporaryDirectory() as folder:
            bundle = Path(folder)
            config = bundle / "_internal" / "configs" / DEFAULT_CONFIG_BASENAME
            config.parent.mkdir(parents=True)
            config.write_text("{}", encoding="utf-8")
            hits = _default_search_paths(bundle / "algorithm.exe")
            self.assertIn(config, hits)
