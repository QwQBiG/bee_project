...
示例用法:
  # 处理巢外视频
  .venv\Scripts\python.exe src\main.py --mode outside --video data/outside.mp4 --output results/

  # 处理巢内视频
  .venv\Scripts\python.exe src\main.py --mode inside --video data/inside.mp4 --output results/

  # 多模态同步处理
  .venv\Scripts\python.exe src\main.py --mode multi --video data/outside.mp4 --video_inside data/inside.mp4 --output results/

  # 使用标注工具
  .venv\Scripts\python.exe src\main.py --mode annotate --video data/video.mp4 --output annotations/

  # 模型训练
  .venv\Scripts\python.exe src\main.py --mode train --config configs/train_config.yaml
        """
    )
...
def run_demo():
    ...
    dirs = ['annotation', 'tracking', 'behavior', 'visualization',
            'inference', 'models', 'utils', 'data', 'docs', 'tools', 'tests']
    for d in dirs:
        status = "[OK]" if (source_root / d).exists() else "[MISSING]"
        print(f"  {status} {d}/")
    for d in ('configs', 'weights'):
        status = "[OK]" if (repository_root / d).exists() else "[MISSING]"
        print(f"  {status} {d}/")
    
    print("\n" + "=" * 60)
    print("演示完成！")
    print("\n使用方法:")
    print("  python src/main.py --mode outside --video <视频路径> --output <输出目录>")
    print("  python src/main.py --mode inside --video <视频路径> --output <输出目录>")
    print("  python src/main.py --mode multi --video <巢外视频> --video_inside <巢内视频> --output <输出目录>")
    print("=" * 60)
