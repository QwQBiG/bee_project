"""
巢外蜜蜂检测与跟踪模块
基于 YOLOv8 检测器和 Ultralytics 官方多目标跟踪器
针对高密度、频繁进出场景优化
"""
...
class OutsideHiveBeeDetector:
    ...
    def __init__(self, model_path: str = "artifacts/models/hive_entrance_bee_yolov8n.onnx",
...
class MotionIoUTracker:
    """轻量级运动与 IoU 匹配器，仅作为无官方跟踪器时的兼容后端。"""
...
class OutsideHiveTracker:
    ...
    def __init__(self, config: Dict):
        ...
        self.detector = OutsideHiveBeeDetector(
            model_path=config.get('model_path', 'artifacts/models/hive_entrance_bee_yolov8n.onnx'),
        ...
        self.requested_tracker_type = str(config.get('tracker_type', 'botsort')).lower()
...
def create_outside_tracker(config: Dict = None) -> OutsideHiveTracker:
    ...
    if config is None:
        config = {
            'model_path': 'artifacts/models/hive_entrance_bee_yolov8n.onnx',
            'conf_threshold': 0.25,
            'iou_threshold': 0.45,
            'device': None,
